...
namespace Careshop\CommunityTalentProduct\Block\Adminhtml\Talent\Edit\Tab;
 ...
 */
class Talent extends Generic implements TabInterface
{
    /**
     * Wysiwyg config
     *
    ...
    protected function _prepareForm()
    {
        ...
        $talent = $this->_coreRegistry->registry('community_talent');

        /** @var Form $form */
        $form = $this->_formFactory->create();

        $form->setHtmlIdPrefix('talent_');
        $form->setFieldNameSuffix('talent');

        $fieldset = $form->addFieldset('base_fieldset', [
            'legend' => __('Talent Information'),
            'class' => 'fieldset-wide'
        ]);
        ...
        $fieldset->addField(
            'product_id',
            \Careshop\CommunityTalentProduct\Block\Adminhtml\Talent\Edit\Tab\Renderer\Product::class,
            [
                'name'  => 'product_id',
                'label' => __('Product'),
                'title' => __('Product')
        ...
        if ($talent->getData('created_at')) {
            $publicDateTime = new \DateTime($talent->getData('created_at'), new DateTimeZone('UTC'));
            ...
            $talent->setData('created_at', $publicDateTime);
        }

        $form->addValues($talent->getData());
        $this->setForm($form);
