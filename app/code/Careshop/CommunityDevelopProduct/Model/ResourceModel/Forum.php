...
use Careshop\CommunityDevelopProduct\Model\Forum as ForumModel;
...
class Forum extends AbstractDb
{
    ...
    {
        $this->_init('community_develop_forum', 'entity_id'); 
    }

    public function getDevelop($product_id)
    {
        $table_develop_forum = $this->getTable('community_develop_forum');
        $select = $this->getConnection()->select();
        $select->from($table_develop_forum);
        $select->where('product_id = ?', $product_id);
        $forum = $this->getConnection()->fetchRow($select);
        return $forum;
    } 
}
