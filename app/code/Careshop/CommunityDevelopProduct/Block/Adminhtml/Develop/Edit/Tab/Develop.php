...
namespace Careshop\CommunityDevelopProduct\Block\Adminhtml\Develop\Edit\Tab;
 ...
 */
class Develop extends Generic implements TabInterface
{
    /**
     * Wysiwyg config
     *
    ...
    protected function _prepareForm()
    {
        ...
        $develop = $this->_coreRegistry->registry('community_develop');

        /** @var Form $form */
        $form = $this->_formFactory->create();

        $form->setHtmlIdPrefix('develop_');
        $form->setFieldNameSuffix('develop');

        $fieldset = $form->addFieldset('base_fieldset', [
            'legend' => __('Develop Information'),
            'class' => 'fieldset-wide'
        ]);
		...
		$fieldset->addField(
            'product_id',
            \Careshop\CommunityDevelopProduct\Block\Adminhtml\Develop\Edit\Tab\Renderer\Product::class,
            [
                'name'  => 'product_id',
                'label' => __('Product'),
                'title' => __('Product')
        ...
        if ($develop->getData('created_at')) {
            $publicDateTime = new \DateTime($develop->getData('created_at'), new DateTimeZone('UTC'));
            ...
            $develop->setData('created_at', $publicDateTime);
        }

        $form->addValues($develop->getData());
        $this->setForm($form);
