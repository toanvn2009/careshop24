...
namespace Careshop\CommunityIdea\Block\Adminhtml\Topic\Edit\Tab;
...
class Idea extends Extended implements TabInterface
{
    ...
    protected function _prepareCollection()
    {
        ...
        $collection = $this->ideaFactory->create()->getCollection();
        $collection->getSelect()->joinLeft(
            ['related' => $collection->getTable('community_idea_topic')],
            'related.idea_id=main_table.idea_id AND related.topic_id=' . (int) $this->getRequest()->getParam('id', 0),
            ['position']
        );
    ...
    {
        $ideas = $this->getRequest()->getPost('topic_ideas', null);
        if (!is_array($ideas)) {
            $ideas = $this->getTopic()->getIdeasPosition();
    ...
    {
        $selected = $this->getTopic()->getIdeasPosition();
    ...
    {
        return $this->getUrl('*/*/ideasGrid', ['id' => $this->getTopic()->getId()]);
    }

    /**
     * @return \Careshop\CommunityIdea\Model\Topic
     */
    public function getTopic()
    {
        return $this->coreRegistry->registry('community_topic');
    }
    ...
    {
        return $this->getUrl('community/topic/ideas', ['_current' => true]);
    }
